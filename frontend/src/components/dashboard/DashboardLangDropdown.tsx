  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="px-2.5 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-300 font-mono text-xs uppercase transition-all border border-[var(--border-default)] flex items-center space-x-1.5"
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-44 bg-[var(--bg-card)] border border-[#E9C349]/30 rounded-2xl shadow-2xl p-1.5 z-50">
          {LANGUAGES.map((lang) => (
            <button
              key={lang.code}
